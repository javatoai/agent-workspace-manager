...
Windows 使用 `%USERPROFILE%\.AgentWorkspaceManager`，macOS 使用 `~/.AgentWorkspaceManager`。0.5.2 的说明文件固定保存在：
...
0.5.2 的 `config.json` 使用严格字符串 schema `"0.5.2"`。顶层仓库和组均为数组，数组顺序就是界面顺序：
...
`agent-workspace.json` 使用严格字符串 schema `"0.5.2"`。创建任务时会继承所属组的 `defaultWorkspaceToolIds`，用户可以在创建页增减。任务本身创建成功后，工具适配器逐项打开；其中一个失败不会回滚 Git 工作区，也不会阻止其他工具。
