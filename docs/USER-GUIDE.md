...
1. 点击“创建研发任务”。
2. 可选地手工填写需求编号或飞书需求链接，或从候选需求中选择。输入需求时，资料目录使用下一步填写的文件夹名称；不会替换该名称。设置中的资料根目录和资料子目录均已配置时，创建页会只读预检并显示预计新建或复用的完整资料路径；任一项未配置时，创建流程不会展示或尝试创建资料目录。资料目录按 Sprint 分组：本地已有可复用目录时始终复用；否则需求只关联一个 Sprint 时直接使用它（无论是否进行中），关联多个时使用唯一进行中的 Sprint。
