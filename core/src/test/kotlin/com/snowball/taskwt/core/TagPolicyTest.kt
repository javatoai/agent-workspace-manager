...
class TagPolicyTest {
    ...
    fun `group and child switches both gate tag creation`() {
        ...
        val enabled = AppConfig(repositories = listOf(repository()), groups = listOf(ServiceGroupConfig("g", "G", true, listOf(service))))
        val disabled = enabled.copy(groups = listOf(enabled.groups.single().copy(createTagEnabled = false)))
    ...
    fun `independent clone uses actual clone branch and clone tag settings`() {
        ...
        val manifest = manifest(workspace)
        val config = AppConfig(repositories = listOf(repository()), groups = listOf(ServiceGroupConfig("g", "G", true, listOf(service))))
