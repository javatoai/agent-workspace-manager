...
class WorkspaceLifecycleIntegrationTest {
    ...
    fun `same-base modules create archive restore and delete one physical worktree`() {
        ...
        val config = AppConfig(
            ...
            groups = listOf(ServiceGroupConfig("shared", "Shared", services = listOf(serviceConfig))),
        )
    ...
    fun `independent clone requires explicit discard for commits on any local branch`() {
        ...
        val config = AppConfig(
            taskRoot = taskRoot.toString(),
            repositories = listOf(repository),
            groups = listOf(ServiceGroupConfig("clone-risk", "Clone Risk", services = listOf(cloneService))),
        )
        val application = TaskApplicationService(
