...
class GitWorkspaceLifecycle(
    ...
    private fun validateConfiguredRepository(config: AppConfig, workspace: ServiceWorkspace): Path {
        ...
        if (configured == null && config.services.containsKey(workspace.repositoryId)) {
            val legacyRepository = Path.of(workspace.repositoryPath).toAbsolutePath().normalize()
            require(git.commonDirectory(legacyRepository).toAbsolutePath().normalize() ==
                git.commonDirectory(Path.of(workspace.worktreePath)).toAbsolutePath().normalize()) {
                "任务 Worktree 不属于记录的主仓库"
            }
