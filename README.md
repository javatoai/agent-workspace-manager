...
Task Worktree Manager（TaskWT）是面向多组、多服务研发任务的桌面管理器。它把每个任务的代码工作区、分支、AGENTS.md 与 UAT Tag 操作集中在一个界面中，并用 Git 安全检查保护用户已有工作。
...
- 任务完成后可通过通用适配器打开 Codex；未来新增 Claude、Cursor 只需注册新的工具适配器。
- TaskWT 自己生成的时间统一使用上海时区的 `yyyy-MM-dd HH:mm:ss` 格式。
