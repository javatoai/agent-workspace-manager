...
```powershell
codex plugin marketplace add https://github.com/javatoai/agent-workspace-manager.git --ref v0.11.1
...
`0.11.0` 使用字符串 schema。仅相同主次版本的 PATCH 可兼容读取；主版本或次版本不同则拒绝读取，且不会迁移或改写旧数据。`0.10.x` 及更早数据不会读取或迁移。
