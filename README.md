...
它适合需要同时修改多个服务、希望使用 Git Worktree 隔离任务、并且会配合 Codex、Cursor 等 Agent 工具开发的人或团队。创建任务后，AWM 会按你选择的服务建立独立工作区、生成任务说明、展示 Git 改动与推送状态，并在需要时构建 UAT Tag。
...
- 团队按业务线把服务分组管理；不同组可以使用不同服务、分支前缀、Agent 说明和 UAT Tag 配置。
- 需要在本地提交前快速确认：哪些文件还没有提交、哪些提交尚未推送、是否可以安全归档或删除任务。

## 它能做什么

- **按任务创建隔离工作区**：标准服务使用 Git Worktree；也可将服务配置为独立克隆。
- **按组管理服务**：为每组维护服务清单、服务排序、分支前缀、Agent 说明、默认打开的工具和 UAT Tag 开关；只有一个组时界面保持简洁。
- **一次覆盖多个服务**：一个任务可选择多个服务；多模块服务按基础分支建立相应的工作区。
...
- **构建与追溯 UAT Tag**：按组和服务配置创建 Tag，保留可复制的构建历史。
- **保护已有工作**：归档、删除和 Tag 操作执行 Git 安全检查；不会 Force Push、自动 Rebase 或自动解决冲突。

## 三分钟上手

1. 在“设置”中选择任务工作区根目录，并添加本地 Git 仓库到一个组。
2. 为服务选择“标准 Worktree”或“独立克隆”，按需配置基础分支、UAT Tag 和开发工具。
3. 点击“创建任务”，填写或选择需求链接、任务名称和分支，再选择需要修改的服务。
4. 在任务详情中打开工作区开始开发；完成后检查未提交/未推送状态，按需构建 UAT Tag、归档或删除任务。
...
0.6.0 只提供桌面应用，不再包含命令行模块或命令行发布包。
...
0.6.x 使用字符串 schema。相同主次版本的 PATCH（如 `0.6.0`、`0.6.1` 与 `0.6.9`）可兼容读取；主版本或次版本不同则拒绝读取，且不会迁移或删除旧数据。
