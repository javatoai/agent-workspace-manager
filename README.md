...
`1.0.x` 使用字符串 schema，当前写入版本为 `1.0.4`。相同主次版本的 PATCH 可兼容读取；主版本或次版本不同则拒绝读取，且不会迁移或改写旧数据。`0.12.x` 及更早数据不会读取、迁移或删除。升级时需先备份数据，手工移除旧配置中的 `requirementDocumentationRoot`，将 schema 改为 `1.0.4`，并在设置页重新保存需求资料根目录与资料子目录。
